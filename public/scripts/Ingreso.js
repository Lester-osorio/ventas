...
function init() {
   ...
   // $("#cboTipoComprobanteIng").change(VerNumSerie);
	...
	$("#btnAgregarArt").click(function(e){
		...
		opt.each(function() {
			AgregarDetalle($(this).val(), $(this).attr("data-nombre"), "", "", "", "1", "1", "1", "1", "1","1");
		})
        ...
        for (var i = 0; i < stock_ing.length; i++) {
            if (stock_ing[i].value !== "") {
                AgregarDetalleRegistrar(idArt[i].value, cod[i].value, serie[i].value, desc[i].value, stock_ing[i].value, 1,
                	 prec_comp[i].value, utilidad[i].value, prec_ventaD[i].value, prec_ventaP[i].value);
            }
        }
    ...
    function GetImpuesto() {
        ...
        $.getJSON("./ajax/GlobalAjax.php?op=GetImpuesto", function(r) {
			    ...
			    $("#Utilidad").html(r.simbolo_moneda + " Utilidad:");
    ...
    */
    ...
    function AgregarDetalle(idart, nombre, codigo, serie, descripcion, stock_ing, stock_act, p_compra, utilidad, p_ventaD, p_ventaP) {
        var detalles = new Array(idart, nombre, codigo, serie, descripcion, stock_ing, stock_act, p_compra, utilidad, p_ventaD, p_ventaP);
        elementos.push(detalles);
        ConsultarDetalles();
    }

    function AgregarDetalleRegistrar(idart, codigo, serie, descripcion, stock_ing, stock_act, p_compra, utilidad, p_ventaD, p_ventaP) {
        var detallesReg = new Array(idart, codigo, serie, descripcion, stock_ing, stock_act, p_compra, utilidad, p_ventaD, p_ventaP);
...
function ListadoIngresos(){ 
    ...
    function cargarDataIngreso(id, serie, numero, impuesto, utilidad, total, idIngreso, Proveedor, tipo_comprobante){
        bandera = 2;
        $("#VerForm").show();
        $("#btnNuevo").hide();
        $("#VerListado").hide();
       ...
       // $("#VerMod").show();
        ...
        $("#txtTotal").val(Math.round(total*100)/100);
		  $("#txtUtilidad").val(Math.round(total*100)/100);
 ...
 function calcularUtilidad(posi){
        
        var utilidad = 0;
        var data = JSON.parse(objinit.consultar());
        for (var pos in data) {
            utilidad += parseFloat(data[pos][8] - data[pos][6]);
        }
		
        calcularIgv();
        calcularSubTotal();
		
        $("#txtUtilidad").val(Math.round(utilidad*100)/100);
        if(posi != null){
          Modificar(posi);
            //alert(pos);
        }
